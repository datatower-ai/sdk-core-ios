...
@interface DTPropertyPluginManager ()
@property (nonatomic, strong) NSMutableArray<id<DTPropertyPluginProtocol>> *plugins;

@end


@implementation DTPropertyPluginManager
...
- (void)registerPropertyPlugin:(id<DTPropertyPluginProtocol>)plugin {
    ...
    // 删除旧的plugin
    for (id<DTPropertyPluginProtocol> object in self.plugins) {
...
- (NSMutableDictionary<NSString *,id> *)currentPropertiesForPluginClasses:(NSArray<Class> *)classes {
    ...
    NSMutableArray<id<DTPropertyPluginProtocol>> *matchResult = [NSMutableArray array];
    // 遍历插件
    for (id<DTPropertyPluginProtocol> obj in plugins) {
        // 遍历筛选目标class
...
- (NSMutableDictionary *)propertiesWithPlugins:(NSArray<id<DTPropertyPluginProtocol>> *)plugins {
    NSMutableDictionary *properties = [NSMutableDictionary dictionary];
    // 获取匹配的插件属性
    dispatch_semaphore_t semaphore;
    for (id<DTPropertyPluginProtocol> plugin in plugins) {
...
- (BOOL)isMatchedWithPlugin:(id<DTPropertyPluginProtocol>)plugin eventType:(DTEventType)type {
    ...
    if (![plugin respondsToSelector:@selector(eventTypeFilter)]) {
        ...
        eventTypeFilter = DTEventTypeTrack | DTEventTypeTrackFirst | DTEventTypeTrackUpdate | DTEventTypeTrackOverwrite;
    } else {
